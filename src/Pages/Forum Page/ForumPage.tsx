...
const MyRooms=({navigation}:any)=>{
    ...
    const[checkRoomPasswordModalVisible,setCheckRoomPasswordModalVisible]=useState<boolean>(false);
    ...
    async function fetchMyRoomsData() {
        ...
        if(currentUserUID){
            await database().ref('rooms').orderByChild("users").on("value", function(snapshot) {
    ...
    const renderRooms=({item}:any)=><RoomCard room={item} onPress={()=>goMessagesPage(item,setRoomForCheckPassword,setCheckRoomPasswordModalVisible,navigation)}></RoomCard>
    return(
        <View style={styles.container}>
            <InputBox iconName='magnify' placeholder='Search in my rooms...' onChangeText={(t)=>searchInMyRooms(t)}></InputBox>
            <Text>MYROOMS</Text>   
            {filteredRoomData ? <FlatList data={filteredRoomData} renderItem={renderRooms} numColumns={2}></FlatList>:<FlatList data={myRoomsData} renderItem={renderRooms} numColumns={2}></FlatList>}
            <CheckRoomPasswordModal isVisible={checkRoomPasswordModalVisible} onClose={handleCheckRoomPasswordModalVisible} room={roomForCheckPassword}></CheckRoomPasswordModal>
        </View>
    )
}
const AllRooms=({navigation}:any)=>{
    ...
    async function fetchRoomData() {
        await database().ref(`rooms/`).on('value', snapshot => {
    ...
    return(
        <View style={styles.container}>
             <InputBox iconName='magnify' placeholder='Search in all rooms...' onChangeText={(t)=>searchInAllRooms(t)}></InputBox>
            {filteredRoomData ? <FlatList data={filteredRoomData} renderItem={renderRooms} numColumns={2}></FlatList>:<FlatList data={roomsData} renderItem={renderRooms} numColumns={2}></FlatList>}
            <FloatingButton onPress={handleCreateRoomVisible}></FloatingButton>
            <CreateRoomModal isVisible={createRoomModalVisible} onClose={handleCreateRoomVisible} onCreateRoom={createARoom} loading={loading}></CreateRoomModal>
            <CheckRoomPasswordModal isVisible={checkRoomPasswordModalVisible} onClose={handleCheckRoomPasswordModalVisible} room={roomForCheckPassword}></CheckRoomPasswordModal>
        </View>
    )
}
...
async function goMessagesPage(room:any,roomPassword:any,modalVisible:any,navigation:any){
        if(room.isPrivate){
            roomPassword(room);
            modalVisible(true);
        }
          ...
          if(currentUserUID){
            await database().ref(`rooms/${room.id}/users`).orderByChild("users").on("value", function(snapshot) {
