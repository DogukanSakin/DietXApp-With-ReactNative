...
const AddDailyConsumptionModal: FC<IModalProps> = ({
  isVisible,
  onClose,
  food,
  addDailyConsumption,
  ...
  function calculateTotalCal(enteredQuantity: any) {
    ...
    if (food) {
      const calculatedTotalCal =
        Math.floor(food.nf_calories) * Math.floor(enteredQuantity);

      setTotalCal(calculatedTotalCal);
    ...
    <Modal
      ...
      <View style={styles.container}>
        {food ? (
          ...
          <Text style={styles.foodNameText}>{foodTEST.food_name}</Text>
        )}

        <InputBox
          iconName="counter"
          placeholder="Enter quantity... (Minimum enter:1)"
          onChangeText={calculateTotalCal}
        />
        {errorVisible ? (
        ...
        <MainButton
          title="Add to daily consumption"
          onPress={handleAddDailyConsumption}
        />
      </View>
    </Modal>
  );
