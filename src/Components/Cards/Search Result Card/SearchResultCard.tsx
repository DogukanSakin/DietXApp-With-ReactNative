...
const SearchResultCard:FC<ICardProps>=({item,viewDetail,addButtonVisible=false,addDailyConsumption,loading=false})=>{
        ...
        <View style={styles.container}>
            <View style={styles.innerContainer}>
                ...
                <View style={styles.textInfoContainer}>
                    ...
                    <Text style={styles.nameText} numberOfLines={1}>{item.food_name}</Text>
                    <Text style={styles.brandText}>{item.brand_name}</Text>
                </View>
                {addButtonVisible ? (
                    <TouchableWithoutFeedback onPress={handleAddDailyConsumption}>
                    <View style={styles.addButtonContainer}>
                        {loading ? <ActivityIndicator color={Colors.iconColor}></ActivityIndicator> :
                        <>
                            <Icon name='hamburger-plus' size={30} color={Colors.iconColor}></Icon>
